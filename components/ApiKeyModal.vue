  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div :class="['w-full max-w-md p-6 rounded-lg shadow-xl', theme === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900']">
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <select
            ...
            :class="['w-full p-3 rounded-lg border focus:ring-2 focus:ring-red-500 outline-none transition-all', theme === 'dark'
        ...
        <div>
          ...
          <input
            ...
            :class="['w-full p-3 rounded-lg border focus:ring-2 focus:ring-red-500 outline-none transition-all', theme === 'dark' ? 'bg-gray-800 border-gray-600 text-white placeholder-gray-500' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-400']"
          />
      ...
      <div :class="['p-6 border-t', theme === 'dark' ? 'border-gray-700 bg-gray-800/50' : 'border-gray-100 bg-gray-50']">
        <button
          @click="handleSave"
          class="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold transition-colors shadow-lg flex items-center justify-center gap-2"
        >
...
const onModelChange = (val: string) => emit('modelChange', val); // Used in template v-if check? No, passed as prop/event

const handleSave = () => {
    if (!key.value.trim()) {
        error.value = t.value.errors.apiKeyEmptyError; // t is computed ref
        return;
    }
